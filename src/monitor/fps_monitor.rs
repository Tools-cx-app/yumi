...
pub async fn start_fps_loop(tx: Sender<DaemonEvent>) -> Result<(), anyhow::Error> {
    ...
    for sym in syms {
        if program
            .attach(Some(sym), 0, "/system/lib64/libgui.so", None)
            .is_ok()
        {
            info!(
                "{}",
                t_with_args("fps-monitor-attached", &fluent_args!("sym" => sym))
            );
            attached_count += 1;
        }
    }
    ...
    let map = unsafe { &mut *bpf_ptr }
        .map_mut("frame_events")
        .expect("frame_events map not found");
                    ...
                    if let Some(arr) = &mut target_pid_arr {
                        ...
                        debug!(
                            ...
                            t_with_args(
                                ...
                                &fluent_args!("old" => last_pid.to_string(), "new" => current_pid.to_string())
                            )
                        );
                    }
                ...
                match buf.read_events(&mut buffers).await {
                    Ok(events) => {
                        for (i, _) in buffers.iter().enumerate().take(events.read) {
                            ...
                            if delta == 0 || event_pid != pid_arc.load(Ordering::Relaxed) {
                                continue;
                            }
    ...
    info!(
        ...
        t_with_args(
            "fps-monitor-started",
            &fluent_args!("filter" => if has_kernel_filter { "active" } else { "disabled" })
        )
    );
